...
class CDisplay():
    ...
    def barebadpixcor(self, images, band, headers=None):
        badpixelfile = "badpixel_%s.fits" %(band.lower())
	    ...
	    img = fixpix(img, badpixelmask, 'cinterp')
	    imgs.append(img)
    ...
    def barecombine(self, imgs, method="average", hdr=None):
        if method == "median":
            comb = imcombine(imgs, mode="median")
        elif method == "average":
            comb = imcombine(imgs, mode="average")

        if hdr is None: hdr = newheader()
        hdr.update('HISTORY','combined %d files' %len(imgs))
        return comb, hdr
    ...
    def baresubtract(self, on_imgs, off_imgs, hdr=None):
        AB = imarith(on_imgs, off_imgs, "-")
        if hdr is None: hdr = newheader()
        hdr.update('HISTORY', 'Flat ON - FLat OFF')
        return AB, hdr
    ...
    def barenormalize(self, img, hdr=None, band="H"):
        if band == "H":
            img = np.divide(img, 3200)
        elif band == "K":
            img = np.divide(img, 1700)

        if hdr is None: hdr = newheader()
        hdr.update('HISTORY', 'Normalized maximum value to unity')
